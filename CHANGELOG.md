...
### 变更
- **临时密码生成改两步式(App 同链路)**:bundle 逆向证实 App 的 keyId/tempKey
  均由服务端签发(`SetService · CreateDeviceSnapkey` → 消息域 Add 固化)。
  `create_snapkey` 现优先该链路(设备侧流水知晓密钥, state 0→1, App 可删),
  设备离线自动回落客户端自产;`snapkey_created` 事件新增 `key_id`/`mode`
  (server|client)字段。
- **生效星期改为系统自动分配(每天)**:生效范围由有效天数决定,移除星期选择
  实体与相关常量(历史 options 残留键自动失效,无需迁移)。

### 新增
- **"最新告警"更名"告警信息"**,状态行精简为 `行为 · 标题`。
- **告警属性结构化**:`alarms` 属性改为字典列表(行为/标题/时间,最新在前)——
  more-info 逐块渲染小表,不截断;原始数据保留在 coordinator.data。
- **属性展示文本国际化(i18n.py)**:词映射随 HA 实例语言产出(zh-Hans/en,
  未收录语言回退 zh-Hans);告警标题为云端数据文本,不翻译;默认语言输出
  与历史中文完全一致。
- **工作模式 select + 传感器(powerMode)**:自动/正常/省电/超级省电四挡;
  设备模型无 powerMode 时显示 unavailable。
- **临时密码列表显示重构**:单属性 `snapkeys` 字典列表(名称/密码/过期时间/
  次数/状态,最新在前,上限 20);实体更名"临时密码列表"(unique_id 不变,
  自动更名无孤儿)。

### 修复
- **keyId 收敛 8 位内(9 位 → App 无法删除的根因)**:随机区间误写导致 9 位
  keyId,创建可过但 App/设备侧删除匹配不上;现与 App 一致(8 位内)。
- **删除对齐 App 包 + 结果核验**:SmartLockSecretDelete 回传列表条目的精确
  11 业务字段;删除后重拉列表核验,未消失发 `snapkey_delete_failed` 事件
  (`snapkey_deleted` 事件新增 `verified` 字段)。state=0 待设备确认的密钥
  App 会本地拦截不发包,处置:HA 删除/唤醒确认/到期自动清理。
- **"次数"误读字段**:次数改读 `number`(-1/0 → 无限次),`effectTimes`
  (有效天数)不再参与次数显示。
- **"最近开门方式/时间"不准**:告警按时间稳定排序;标题解析开门方式
  (指纹/卡片/人脸/临时密码/远程/机械钥匙);时间字段优先级明确
  (localTime 优先,pTimestamp 回退);keyType=0 等合法 falsy 值修复。
- **MQTT 重连被拒风暴("0 bytes read" 退避刷屏)**:根因为关闭连接不发
  DISCONNECT 导致服务端会话滞留、同 clientId 重连被拒;现优雅释放会话,
  连接失败不再泄漏半开客户端。
