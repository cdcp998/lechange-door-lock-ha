...
- **控制开关**：童锁、呼叫转接（`iot.control.SetProperties`）。
  - **童锁**：真实载体为 `sdl_inOpenDoorModel`(171700, 2=童锁/1=普通)——官方链路
    **直接 `SetProperties`**，不唤醒/不取流/不 await 在线，允许休眠设备直接写
    ("下次唤醒生效")。`child_lock`(120000) 已过期且语义与 171700 相反/混乱，已弃用
    (只认 171700 单一真源)。
- **🚪 开门设置**(App「开门设置」页属性面, `iot.control.Get/SetProperties`):
  - **开关**：门锁自动上锁、门未关提醒、人脸自动感应开门、回家欢迎灯、和家智话远程开门等;
  - **选择**：自动上锁时间(15/30/45/60/180s)、门未关提醒时间、防误开时间、提示音音量、室内开门模式;
  - **只读状态**：强制锁定 / 电子反锁 / 组合开门 —— ⚠️ 高危属性**只读展示、不提供写控制**(开启后仅管理员可开/普通密钥失效, 需在 App 内操作);
  - **能力门控**：实体仅在本机物模型声明 **且** GetProperties 实测返回 **且** 型号
    可写(accessMode 含 w)时创建 —— 云端按设备能力过滤(App 同语义), 本机不支持
    的属性不建实体, 不产生永远 unavailable 的"僵尸实体"; 已确认清单持久化, 重启不丢;
  - **int 写入铁律**：SetProperties 值必须 int(字符串被设备拒 19999); 设备休眠时
    改值写被拒 → 自动唤醒后重试一次; 另有「唤醒设备」按钮手动兜底。
